...
export const getComments = async (req, res, next) => {
    ...
    try {
        ...
        const comments = await Comment.find().skip(startIndex).limit(limit).sort({ updateAt: sortDirection });
        ...
        return res.status(200).json({ comments, totalComments, lastMonthComment });
    } catch (error) {
        next(error); 
    }
}
